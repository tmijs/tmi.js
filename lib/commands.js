...
function followersonly(channel, minutes) {
	...
	minutes = minutes ?? 30;
	return this._sendCommand(null, channel, `/followers ${minutes}`, (res, rej) =>
...
function followersonlyoff(channel) {
	channel = _.channel(channel);
	return this._sendCommand(null, channel, '/followersoff', (res, rej) =>
...
function part(channel) {
	channel = _.channel(channel);
	return this._sendCommand(null, null, `PART ${channel}`, (res, rej) =>
...
function r9kbeta(channel) {
	...
	return this._sendCommand(null, channel, '/r9kbeta', (resolve, reject) =>
		this.once('_promiseR9kbeta', err => !err ? resolve([ channel ]) : reject(err))
	);
}
...
function r9kbetaoff(channel) {
	...
	return this._sendCommand(null, channel, '/r9kbetaoff', (resolve, reject) =>
		this.once('_promiseR9kbetaoff', err => !err ? resolve([ channel ]) : reject(err))
	);
}
...
function slow(channel, seconds) {
	channel = _.channel(channel);
	seconds = seconds ?? 300;
	return this._sendCommand(null, channel, `/slow ${seconds}`, (res, rej) =>
...
function slowoff(channel) {
	channel = _.channel(channel);
	return this._sendCommand(null, channel, '/slowoff', (res, rej) =>
...
module.exports = {
	...
	action(channel, message) {
		...
		return this._sendMessage(this._getPromiseDelay(), channel, message, (res, _rej) =>
	...
	ban(channel, username, reason) {
		...
		reason = reason ?? '';
		return this._sendCommand(null, channel, `/ban ${username} ${reason}`, (res, rej) =>
	...
	clear(channel) {
		channel = _.channel(channel);
		return this._sendCommand(null, channel, '/clear', (res, rej) =>
	...
	color(channel, newColor) {
		...
		return this._sendCommand(null, '#tmijs', `/color ${newColor}`, (res, rej) =>
	...
	commercial(channel, seconds) {
		channel = _.channel(channel);
		seconds = seconds ?? 30;
		return this._sendCommand(null, channel, `/commercial ${seconds}`, (res, rej) =>
	...
	deletemessage(channel, messageUUID) {
		channel = _.channel(channel);
		return this._sendCommand(null, channel, `/delete ${messageUUID}`, (res, rej) =>
	...
	emoteonly(channel) {
		channel = _.channel(channel);
		return this._sendCommand(null, channel, '/emoteonly', (res, rej) =>
	...
	emoteonlyoff(channel) {
		channel = _.channel(channel);
		return this._sendCommand(null, channel, '/emoteonlyoff', (res, rej) =>
	...
	host(channel, target) {
		...
		return this._sendCommand(2000, channel, `/host ${target}`, (res, rej) =>
	...
	join(channel) {
		...
		return this._sendCommand(undefined, null, `JOIN ${channel}`, (res, rej) => {
	...
	mod(channel, username) {
		channel = _.channel(channel);
		username = _.username(username);
		return this._sendCommand(null, channel, `/mod ${username}`, (res, rej) =>
	...
	say(channel, message) {
		...
		if((message.startsWith('.') && !message.startsWith('..')) || message.startsWith('/') || message.startsWith('\\')) {
			...
			if(message.slice(1, 4) === 'me ') {
				...
				return this._sendCommand(null, channel, message, (res, _rej) =>
					// At this time, there is no possible way to detect if a message has been sent has been eaten
					// by the server, so we can only resolve the Promise.
					res([ channel, message ])
				);
	...
	subscribers(channel) {
		channel = _.channel(channel);
		return this._sendCommand(null, channel, '/subscribers', (res, rej) =>
	...
	subscribersoff(channel) {
		channel = _.channel(channel);
		return this._sendCommand(null, channel, '/subscribersoff', (res, rej) =>
	...
	timeout(channel, username, seconds, reason) {
		...
		seconds = seconds ?? 300;
		reason = reason ?? '';
		return this._sendCommand(null, channel, `/timeout ${username} ${seconds} ${reason}`, (res, rej) =>
	...
	unban(channel, username) {
		channel = _.channel(channel);
		username = _.username(username);
		return this._sendCommand(null, channel, `/unban ${username}`, (res, rej) =>
	...
	unhost(channel) {
		channel = _.channel(channel);
		return this._sendCommand(2000, channel, '/unhost', (res, rej) =>
	...
	unmod(channel, username) {
		channel = _.channel(channel);
		username = _.username(username);
		return this._sendCommand(null, channel, `/unmod ${username}`, (res, rej) =>
	...
	unvip(channel, username) {
		channel = _.channel(channel);
		username = _.username(username);
		return this._sendCommand(null, channel, `/unvip ${username}`, (res, rej) =>
	...
	vip(channel, username) {
		channel = _.channel(channel);
		username = _.username(username);
		return this._sendCommand(null, channel, `/vip ${username}`, (res, rej) =>
	...
	vips(channel) {
		channel = _.channel(channel);
		return this._sendCommand(null, channel, '/vips', (res, rej) =>
	...
	whisper(username, message) {
		...
		return this._sendCommand(null, '#tmijs', `/w ${username} ${message}`, (_res, rej) =>
