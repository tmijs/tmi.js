            ...
            switch(message.command) {
                ...
                case "CLEARCHAT":
                    ...
                    if (message.params.length > 1) {
                        // FIXME: Unescape the ban reason.
                        var reason = _.get(message.tags["ban-reason"], null);
                        var duration = _.get(message.tags["ban-duration"], null);
                        var cause = reason !== null ? `, reason: ${reason}` : ".";

                        if (_.isNull(duration)) { this.log.info(`[${channel}] ${message.params[1]} has been banned${cause}`); }
                        else { this.log.info(`[${channel}] ${message.params[1]} has been timed out for ${duration} seconds${cause}`); }
...
client.prototype._sendMessage = function _sendMessage(delay, channel, message, fn) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!this.rateLimiter.canSendMessage()) { return reject("Ratelimit reached."); }
