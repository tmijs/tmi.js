...
client.prototype.handleMessage = function handleMessage(message) {
    if (message !== null) {
        var channel = typeof message.params[0] === "undefined" ? null : _.channel(message.params[0]);
            ...
            switch(message.command) {
                ...
                case "HOSTTARGET":
                    ...
                    if (message.params[1].split(" ")[0] === "-") {
                        ...
                        this.emit("unhost", channel, message.params[1].split(" ")[1] || "0");
                ...
                case "ROOMSTATE":
                    ...
                    if (message.tags.hasOwnProperty("slow") && !message.tags.hasOwnProperty("subs-only")) {
                        if (typeof message.tags.slow === "boolean") {
                            ...
                            this.emit("slow", channel, false, "0");
                            this.emit("slowmode", channel, false, "0");
                            this.emit("_promiseSlowoff", null);
                        } else {
                            this.log.info(`[${channel}] This room is now in slow mode.`);
                            this.emit("slow", channel, true, message.tags.slow);
                            this.emit("slowmode", channel, true, message.tags.slow);
                ...
                case "PRIVMSG":
                    ...
                    // Message from JTV..
                        ...
                        if (message.params[1].match(/^\u0001ACTION ([^\u0001]+)\u0001$/)) {
                            ...
                            this.emit("action", channel, message.tags, message.params[1].match(/^\u0001ACTION ([^\u0001]+)\u0001$/)[1], false);
                            this.emit("message", channel, message.tags, message.params[1].match(/^\u0001ACTION ([^\u0001]+)\u0001$/)[1], false);
                        }
                        // Message is a regular message..
                        else {
                            message.tags["message-type"] = "chat";
                            this.log.info(`[${channel}] <${message.tags.username}>: ${message.params[1]}`);
                            this.emit("chat", channel, message.tags, message.params[1], false);
                            this.emit("message", channel, message.tags, message.params[1], false);
                        }
                    }
                    break;
...
client.prototype.connect = function connect() {
    return new Promise((resolve, reject) => {
        ...
        if (this.server === "RANDOM" || typeof this.opts.connection.cluster !== "undefined") {
            var cluster = typeof this.opts.connection.cluster === "undefined" ? "aws" : this.opts.connection.cluster;
...
client.prototype._onOpen = function _onOpen() {
    if (!_.isNull(this.ws) && this.ws.readyState === 1) {
        ...
        this.username = typeof this.opts.identity.username === "undefined" ? _.justinfan() : this.opts.identity.username;
        this.password = typeof this.opts.identity.password === "undefined" ? "SCHMOOPIIE": _.password(this.opts.identity.password);
    ...
    return new Promise((resolve, reject) => {
        if (!_.isNull(this.ws) && this.ws.readyState !== 2 && this.ws.readyState !== 3 && !this.getUsername().includes("justinfan")) {
            ...
            if (message.match(/^\u0001ACTION ([^\u0001]+)\u0001$/)) {
                ...
                this.emit("action", _.channel(channel), this.userstate[_.channel(channel)], message.match(/^\u0001ACTION ([^\u0001]+)\u0001$/)[1], true);
                this.emit("message", _.channel(channel), this.userstate[_.channel(channel)], message.match(/^\u0001ACTION ([^\u0001]+)\u0001$/)[1], true);
            }
            else {
                this.userstate[_.channel(channel)]["message-type"] = "chat";
                this.log.info(`[${_.channel(channel)}] <${this.getUsername()}>: ${message}`);
                this.emit("chat", _.channel(channel), this.userstate[_.channel(channel)], message, true);
                this.emit("message", _.channel(channel), this.userstate[_.channel(channel)], message, true);
            }
            fn(resolve, reject);
        } else {
