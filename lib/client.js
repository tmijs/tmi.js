	...
	// Messages with "tmi.twitch.tv" as a prefix..
		...
		switch(message.command) {
			...
			case "USERNOTICE":
				...
				switch(msgid) {
					...
					case "raid":
						...
						this.emit("raided", channel, username, viewers, userstate);
						break;
				}
			...
			case "USERSTATE":
				...
				this.emit("_promiseSay", null, channel, message.tags);
				break;
...
client.prototype._sendMessage = function _sendMessage(delay, { channel, message, tags }, fn) {
	// Promise a result..
	return new Promise((resolve, reject) => {
		...
		if(message.length >= 500) {
			...
			setTimeout(() => {
				this._sendMessage(delay, { channel, message: msg[1], tags });
			}, 350);
		}
		...
		if(!tags.hasOwnProperty('client-nonce')) {
			tags['client-nonce'] = _.clientNonce();
		}
		var formedTags = parse.formTags(tags);

		this.ws.send(`${formedTags} PRIVMSG ${chan} :${message}`);
