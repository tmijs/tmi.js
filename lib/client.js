...
client.prototype.connect = function connect() {
    ...
    return new Promise(function (resolve, reject) {
        self.reconnect = typeof self.opts.connection.reconnect === "undefined" ? false : self.opts.connection.reconnect;
        self.server = typeof self.opts.connection.server === "undefined" ? "RANDOM" : self.opts.connection.server;
        self.port = typeof self.opts.connection.port === "undefined" ? 443 : self.opts.connection.port;
        self.reconnectTimer = self.reconnectTimer + 10000;
        if (self.reconnectTimer >= 60000) {
            self.reconnectTimer = 60000;
        }
...
client.prototype.disconnect = function disconnect() {
    ...
    return new Promise(function (resolve, reject) {
        if (!_.isNull(self.ws) && self.ws.readyState !== 3) {
            self.wasCloseCalled = true;
            self.log.info("Disconnecting from server..");
            self.ws.close();
            resolve();
        } else {
            self.log.error("Cannot disconnect from server. Socket is not opened or connection is already closing.");
            reject();
        }
    });
};
