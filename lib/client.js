            ...
            switch(message.command) {
                ...
                case "NOTICE":
                    ...
                    switch(msgid) {
                        ...
                        default:
                            if (message.params[1].includes("Login unsuccessful") || message.params[1].includes("Login authentication failed")) {
                                this.wasCloseCalled = false;
                                this.reconnect = false;
                                this.reason = message.params[1];
                                this.log.error(this.reason);
                                this.ws.close();
                            }
                            else if (message.params[1].includes("Error logging in") || message.params[1].includes("Improperly formatted auth")) {
                                this.wasCloseCalled = false;
                                this.reconnect = false;
                                this.reason = message.params[1];
                                this.log.error(this.reason);
