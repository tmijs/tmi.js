    ...
    if (message !== null) {
        ...
        // Messages with "tmi.twitch.tv" as a prefix..
            ...
            switch(message.command) {
                ...
                case "372":
                    ...
                    for (var i = 0; i < this.opts.channels.length; i++) {
                        var self = this;
                ...
                case "USERSTATE":
                    ...
                    if (!contains(this.getUsername(), "justinfan") && !this.userstate[utils.normalizeChannel(message.params[0])]) {
        ...
        // Anything else..
            ...
            switch(message.command) {
                ...
                case "JOIN":
                    if (contains(this.getUsername(), "justinfan") && this.username === message.prefix.split("!")[0]) {
                        this.lastJoined = message.params[0];
                        this.channels.push(message.params[0]);
                        this.log.info("Joined " + message.params[0]);
...
client.prototype._openConnection = function _openConnection() {
    server.isWebSocket(this.server, this.port, (accepts) => {
        ...
        // Server is not accepting WebSocket connections..
            ...
            if (this.reconnect) {
                this.log.error("Server is not accepting WebSocket connections. Reconnecting in " + this.reconnectTimer / 1000 + " seconds..");
                this.emit("reconnect");
                setTimeout(() => { this.connect(); }, this.reconnectTimer);
            } else {
                this.log.error("Server is not accepting WebSocket connections.");
            }
        }
    });
};
...
client.prototype._onClose = function _onClose() {
    ...
    // Got disconnected from server..
        ...
        if (this.reconnect) {
            this.log.error("Sorry, we were unable to connect to chat. Reconnecting in " + this.reconnectTimer / 1000 + " seconds..");
            this.emit("reconnect");
            setTimeout(() => { this.connect(); }, this.reconnectTimer);
        } else {
...
client.prototype._sendMessage = function _sendMessage(channel, message) {
    // Promise a result..
    return new Promise((resolve, reject) => {
        if (!_.isNull(this.ws) && this.ws.readyState !== 2 && this.ws.readyState !== 3 && !contains(this.getUsername(), "justinfan")) {
