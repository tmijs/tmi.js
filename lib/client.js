		...
		switch(message.command) {
			...
			case "PRIVMSG":
				...
				if(message.tags.username === "jtv") {
					...
					messageContent = actionMessage ? actionMessage[1] : msg;
					// Check for Bits prior to actions message
					if (message.tags.hasOwnProperty("bits")) {
						this.emit("cheer", channel, message.tags, messageContent);
					}
					else {
						if(actionMessage) {
							this.log.info(`[${channel}] *<${message.tags.username}>: ${messageContent}`);
							this.emits(["action", "message"], [
								[channel, message.tags, messageContent, false]
							]);
						}
