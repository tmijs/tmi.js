	...
	// Messages with "tmi.twitch.tv" as a prefix..
		...
		switch(message.command) {
			...
			case "USERSTATE":
				...
				const modIndex = this.moderators[this.lastJoined].indexOf(this.username);

				if (modIndex === -1 && message.tags["user-type"] === "mod") {
					this.moderators[this.lastJoined].push(this.username);
					this.log.info('Moder added ' + JSON.stringify({channel: this.lastJoined, username: this.username}));
				} else {
					this.moderators[this.lastJoined].splice(modIndex, 1);
					this.log.info('Moder removed ' + JSON.stringify({channel: this.lastJoined, username: this.username}));
				}
...
client.prototype._sendMessage = function _sendMessage(delay, channel, message, fn) {
	...
	return new Promise((resolve, reject) => {
		...
		if (_.isNull(this.ws) || this.ws.readyState !== 1) {
			return reject(new Error("Not connected to server - " + JSON.stringify({delay, channel, message})));;
