...
client.prototype.handleMessage = function handleMessage(message) {
		...
		switch(message.command) {
			...
			case 'USERNOTICE':
				...
				switch(msgid) {
					...
					// Handle raid
					case 'raid': {
						let username = tags['msg-param-displayName'] || tags['msg-param-login'];
				...
				if(!_.hasOwn(message.tags, 'subs-only')) {
					...
					if(_.hasOwn(message.tags, 'followers-only')) {
						if(message.tags['followers-only'] === '-1') {
							...
							let enabled = [ channel, true, minutes ];
	...
	// Anything else..
		...
		switch(message.command) {
			...
			case 'PART': {
				var isSelf = false;
				let nick = message.prefix.split('!')[0];
...
client.prototype._updateEmoteset = async function _updateEmoteset(sets) {
