...
const _ = module.exports = {
	...
	toNumber(num, precision) {
		...
		const factor = Math.pow(10, isFinite(precision) && !isNaN(parseFloat(precision)) ? precision : 0);
	...
	splitLine(input, length) {
		let lastSpace = input.slice(0, length).lastIndexOf(' ');
		// No spaces found, split at the very end to avoid a loop..
		if(lastSpace === -1) {
			lastSpace = length - 1;
		}
		return [ input.slice(0, lastSpace), input.slice(lastSpace + 1) ];
	},
